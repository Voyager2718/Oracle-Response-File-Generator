...
string getNetworkInterfaceList(string itemName)
{
    ...
    do
    {
        ...
        {                                                        // List networks (No local and virtual networks)
            ...
            default:
                result += ":3,";
                break; // 1 for public, 1 for ASM & private, others for nothing.
            }
            i++;
        }
    ...
    while (true)
    {
        try
        {
            ...
            while (d)
            {
                ...
                { // Read physical device if it is a link.
                    char buffer[1024];
...
Generate response file by local configurations. "
         << VERSION << "\n\n\
...
int main(int argc, char *argv[])
{
    ...
    while ((opt = getopt(argc, argv, "t:o:n:sh")) != -1)
    {
        ...
        {
        case 't':
            templateLocation = string(optarg);
            break;
        case 'o':
            outputLocation = string(optarg);
            break;
        case 'n':
